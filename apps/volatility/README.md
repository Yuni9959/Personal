...
최근 완료 세션 참고값은 승인된 MNQ 출처, 세션 종료 전 마지막 5분 구간,
원천시각 96시간 이내, 원천일과 현재가 같은 weekly 기준 기간이라는 조건을
모두 만족할 때만 열린다. 이 상태는 `referenceOnly`이며 오늘 시세가 아니다.
따라서 가격표 복기만 허용하고 ATR·포지션·손절·실전 계산에는 값을 전달하지
않는다. 조건을 벗어난 stale·불완전 세션·주간 기준 불일치는 동적 pt·가격을
`—`로 잠근다. 수동 입력 패널은 기본적으로 닫혀 있고 사용자가 직접 펼친 뒤
실제 MNQ 월물 전 필드와 확인란을 입력해야 한다.
...
조회나 현재 시세처럼 사용하지 않는다. 완료 시각만 지났을 뿐 마지막 5분 구간이
없거나, 새 weekly 기준으로 이전 주 값을 다시 계산해야 하는 캐시는 표시하지
않는다.
