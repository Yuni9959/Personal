...
Yahoo Finance의 약 10분 지연 `MNQ=F` chart 응답을 받는다. 60초 안의 반복
요청은 기기 로컬 cooldown으로 막고, 예약 실행·주기 폴링·focus 갱신·
백그라운드 갱신은 하지 않는다.
...
Jina 외부·Yahoo 내부 JSON은 각각 512 KiB를 넘으면 읽기를 중단한다. 429는 서버의
