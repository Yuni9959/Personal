...
4. `MNQ=F`가 유효 JSON으로 데이터 없음이라고 답한 경우에만 `NQ=F`를
   한 번 더 확인한다. 429·HTML·네트워크·timeout 오류에서는 추가 호출하지
   않는다. `NQ=F`가 사용되면 이를 **NQ 대체 프록시**로
   분명히 표시하고 자동 계산을 잠근다. 실제 MNQ 월물로 조용히 간주하지
   않는다.
...
페이지 진입 또는 버튼 클릭
        ↓  한 번만 요청
Yahoo chart JSON (`MNQ=F`, 데이터 없음일 때만 `NQ=F` 확인)
