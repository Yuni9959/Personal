...
1. Volatility 페이지에 접속하면 이 기기에 저장된 공급자 스냅샷을 현재 시각,
   ...
   않는다.
2. 평일 최초 진입에는 브라우저가 Jina Reader를 통해 Yahoo `MNQ=F`의 최근
   30일치 5분봉 지연 시세를 한 번 요청한다. 주말 최초 진입은 로컬 NQ를 먼저
   사용하지만, 사용자가 누르는 **오늘 시세 새로고침**은 요일과 관계없이 원격을
   먼저 확인한다. 새 원격·로컬·현재 화면 후보 중 원천시각이 가장 최근인 것을
   선택하므로 실패하거나 더 오래된 응답이 와도 표시값을 되돌리지 않는다.
...
5. Chicago 기준 정규 주말 휴장에는 정적 앱에 동기화된 로컬 `NQ=F` 최근 완료
   ...
   재호출이 아니며, MNQ가 아니므로 최대 96시간의 `referenceOnly` 원시 가격과
   현재 주간 상승·하락 환산선, 자동 ATR 포지션 위험 복기에만 쓰고 손절·실전
   계산은 열지 않는다. 위험 화면에는 마지막 관측 참고값임을 명시한다.
6. 세션 시작 직후 1~2개의 완전-null 5분봉은 Yahoo의 지연 생성 상태로 구분해
   최신가와 완료봉 지표를 참고용으로 허용하되 시가 기반 계산은 잠근다. 현재
   세션의 완료된 중간 5분봉이 정확히 1개 완전-null이지만 Yahoo meta와
   가격 무결성 교차검증을 통과하면 범위와 안전측 계산을 유지한다. 결손 이후
   최신 연속 완료봉이 14개 이상이면 그 구간에서 Wilder ATR을 다시 시작한다.
   시작 3개 이상·마지막·부분-null, 실제 timestamp gap, 중간 둘 이상 null 또는 가격
   교차검증 실패는 전체 자동 계산을 잠근다.
7. 캐시 또는 새 응답이 승인된 출처이고, 현재가 세션 종료 뒤이며, 마지막
   관측값이 종료 전 5분 이내이고 원천시각이 96시간 이내면 **최근 완료 세션
   참고값**으로 표시한다. 원천일이 이전 주여도 활성 주간 기준을 최근 세션
   시가에 환산하고 그 원천 기준시각을 함께 표시한다. 이때
   `referenceOnly=true`, `calculationAllowed=false`로 두어 일반 손절·실전 계산은
   금지한다. 검증된 자동 ATR이 있으면 방향·체결가격·체결시간의 3입력 포지션
   위험 복기만 별도로 허용한다. 종료 시각만 지났지만 마지막 5분
   구간이 없거나 96시간을 넘기면 표시하지 않는다. 자동으로 받은 MNQ
...
페이지 진입 또는 버튼 클릭
        ↓  한 번만 확인
주말 최초 진입: 동기화된 `data/local-nasdaq-snapshot.json` 우선
평일 최초 진입·버튼 클릭: Jina Reader (`r.jina.ai`, 인증정보 없음) 우선
        ↓  `https://query2.finance.yahoo.com/v8/finance/chart/MNQ=F` 전달, 실패 시 로컬 NQ
        ↓  Yahoo chart JSON (`MNQ=F` 최근 30일·5분봉을 한 번만 확인)
        ↓  15초 timeout · 외부/내부 각 512 KiB · URL/provenance 검증
CME 세션 재구성 (America/Chicago 17:00~익일 16:00)
        ↓  5분 bucket · null · OHLC · tick · 원천시각 검증
지연 참고 스냅샷
        ├─ 진행 세션·≤25분·완전: 범위·안전측·완료봉 자동 지표
        ├─ 진행 세션·≤25분·시작 1~2봉 null: 최신가·자동 지표 참고, 시가 계산 잠금
        ├─ 진행 세션·≤25분·1봉 결손 + 가격 메타 일치: 범위·안전측 + 결손 이후 연속봉 ATR
        ├─ 완료 세션 + 종료 마지막 5분 + ≤96h: 기준시각 + 참고 가격표 + 3입력 위험 복기
...
- 세션 시작의 완전-null은 최대 2개까지만 별도 참고 모드로 허용한다. 시작 3개
  이상·마지막·부분-null, 실제 timestamp gap, 중간 둘 이상의 완전-null 또는
...
- Jina 외부 envelope는 `code===200`, `status===200|20000`, `data.url`,
  ...
  30일 차이의 `period1`·`period2`, `includePrePost=true`, `events=div,splits` query가
...
- 이전 캐시를 성공한 새 조회처럼 표시하지 않는다. 시작 시 다시 검증한 뒤
  승인된 출처·완료 세션·종료 전 마지막 5분·원천시각 96시간 이내를 모두
  만족할 때만 `referenceOnly` 참고값으로 표시한다. 활성 주간 기준이 있으면
  이전 주 최근 시가에도 읽기 전용 환산선을 기준시각과 함께 표시한다. 이 상태는
  `calculationAllowed=false`이며 일반 가격선·손절·실전 계산에는 소비하지 않는다.
  단, 출처와 관측시각을 명시한 자동 지표·3입력 포지션 패턴 복기는 참고용으로
  허용한다.
...
- 화면을 열어 둔 중에도 로컬 만료 타이머·focus·pageshow에서 기존 값의
  ...
  96시간을 모두 만족하면 가격표를 참고용으로만 유지한다. 주간 기준이 만료되면
  원시 완료 세션 값만 남기고 상승·하락 환산선은 잠근다.
- 포지션의 손절 ATR은 방향·진입가·진입시각과 출처시각을 묶어 한 번 고정하며,
  새 시세 때문에 조용히 이동시키지 않는다.
- 최근 30일 완료 5분봉으로 EMA50·EMA200·RSI14·ATR(14)과 최근 20세션
  ATR% 백분위를 계산한다. 진입 뒤 차트 구간에서는 MFE·MAE를 산출하고, 797개
  완결 거래의 4군집 특성과 대조한 현재 패턴 **경향**과 주의점을 표시한다.
  수량·추가진입·미래 결과가 없는 현재 포지션을 과거 GMM의 정확한 예측이나
  손실 확률로 표현하지 않는다.
...
| `apps/volatility/js/snapshot-policy.js` | 25분 현재 시세·96시간 완료 세션·종료 5분·최근 시가 환산선·심볼의 순수 fail-closed 판정 |
...
| `apps/volatility/js/weekly-reference.generated.js` | 실행 당일 한국 날짜와 최신 일봉으로 생성된 당주 평균·q25·OOS 계약 |
| `apps/volatility/tools/build-weekly-reference.mjs` | `nasdaq_daily.csv`에서 월요일 기준 직전 5년·selection·52주 holdout을 재계산하는 생성기 |
...
- 장중 종가 방향을 모르는 현재 주간 ex-ante q25 기본선은 상승
  `+0.359538123%`, 하락 `-0.294869926%`이며 최종 미사용 52주의
  **가격선 도달률**은 각각 74.31%(188/253), 73.12%(185/253)다.
- 종가가 양봉·음봉이었다고 사후에 나눈 조건부 q25 복기선은
  `+0.711632208%`, `-0.808562312%`이며 매주 재적합한 정책의 조건부
  도달률은 80.74%(109/135), 77.12%(91/118)다.
