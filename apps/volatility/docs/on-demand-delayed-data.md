...
6. 세션 시작 직후 1~2개의 완전-null 5분봉은 Yahoo의 지연 생성 상태로 구분해
   최신가와 완료봉 지표를 참고용으로 허용한다. 첫 유효 5분봉 시가는 `첫 관측
   기준가 · 공식 시가 아님`으로 명시해 상단 주간 기준표만 환산하고, 하단 실전
   시가 기반 계산은 잠근다. 현재
...
7. 캐시 또는 새 응답이 승인된 출처이고, 현재가 세션 종료 뒤이며, 마지막
   ...
   시가에 환산하고 그 가격의 첫 관측시각을 함께 표시한다. 세션 마지막 봉
   시각을 시가의 기준시각으로 사용하지 않는다. 로컬 완료 세션도 시작 결손은
   최대 2봉만 허용해 첫 유효 봉을 공식 시가가 아닌 기준가로 명시한다. 이때
   `referenceOnly=true`, `calculationAllowed=false`로 두어 일반 손절·실전 계산은
   금지한다. 검증된 자동 ATR이 있으면 현재 포지션 위험 복기만 별도로 허용한다.
   종료 시각만 지났지만 마지막 5분
...
지연 참고 스냅샷
        ...
        ├─ 진행 세션·≤25분·시작 1~2봉 null: 첫 관측 기준가로 상단 표 환산, 실전 계산 잠금
...
- 로컬 동기화 도구는 기본 `C:\Users\tmddb\Desktop\quant\data\nasdaq_5m.csv`의
  정확한 5분봉·0.25 tick·OHLC·시작 2봉 이내의 첫 관측·종료 전 마지막 봉·
  중복과 누락 수를 검증한다. 배포 스냅샷에는 절대경로를 넣지 않고 원본 파일명·
  SHA-256·행수와 첫·마지막 관측시각만 기록한다.
...
- 이전 캐시를 성공한 새 조회처럼 표시하지 않는다. 시작 시 다시 검증한 뒤
  ...
  단, 출처와 관측시각을 명시한 자동 지표·현재 포지션 패턴 복기는 참고용으로
  허용한다.
