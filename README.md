...
| Volatility 시세·기준 | 버튼은 주말에도 Yahoo를 먼저 확인하고 원격·로컬·기존값 중 가장 최근 원천시각을 유지; 주간 분석 기준은 `nasdaq_daily.csv`와 실행 당일 한국 날짜로 자동 생성 |
...
| PWA 캐시 | `v3.9.0-volatility-live-patterns.1` — 최신 후보 선택과 자동 차트 지표·4패턴 경향 포함 |
...
현재 포지션에는 방향·체결가격·체결시간만 입력합니다. 현재가와 최신 연속
완료 5분봉의 Wilder ATR(14)은 자동으로 반영됩니다. 앱은 `현재 불리한 변동`,
`1시간 손실`, `4시간·−2.25 ATR`, `12시간·−1.5 ATR`, `24시간 보유`의
다섯 진행 단계를 각 행에서 판정하고 위험 단계에 따라 축소·청산 검토를
안내합니다. 계약 수를 받지 않으므로 기존 5·7계약 규칙은 자동 판정에서
제외했습니다. 이는 과거 패턴 기반 자기 규칙 점검이지 미래 손실 확률이나
자동 주문이 아닙니다.
...
보여 줍니다. 현재 포지션의 수량·추가진입·최종 결과가 없으므로 이는 정확한
GMM 재분류나 손실 확률이 아니라 설명 가능한 경향 판정입니다.
...
3입력 포지션 위험 복기에만 출처와 기준시각을 표시해 사용할 수 있습니다.
...
봉은 최대 2개까지 시가 계산을 잠근 참고 모드로 허용합니다. 시작 3개 이상이나
중간 둘 이상이 누락되거나
...
6. **Volatility 패턴 경향 실사용 검증**: 자동 4패턴 경향과 MFE·MAE 경고를
   실제 종료 거래에 연결해 오분류 사례를 기록하고, 표본이 충분해지면 수량·
   추가진입 특징을 포함한 저장 모델로 교체합니다.
