...
| Volatility 시세·기준 | 새 시세가 비면 최근 96시간 완료 세션의 기준시각과 상승·하락 읽기 전용 환산선을 표시; 주간 분석 기준은 `nasdaq_daily.csv`와 실행 당일 한국 날짜로 자동 생성 |
| Volatility 포지션 경고 | 방향·체결가격·체결시간 3개만 입력하면 현재가와 최신 연속 5분봉 ATR(14)을 자동 반영해 5단계 위험과 축소·청산 검토를 표시 |
| 자동 검증 | `npm run test:release` 성공 — Node 209/209 및 Chromium·오프라인·320/390px 검증 통과 |
| 배포 공급망 | GitHub Actions 5개를 검증된 전체 커밋 SHA로 고정; Dependabot 주간 업데이트 제안; Linux CI Chrome 경로 고정 |
| PWA 캐시 | `v3.8.0-volatility-three-input-risk.1` — 3입력·자동 ATR 포지션 위험 판정과 최근 완료 세션 참고 포함 |
...
[Jina Reader](https://jina.ai/reader/)를 통해 `MNQ=F` 최근 5일치 5분봉 지연
프록시를 best-effort로 조회합니다. 시가 유무와 관계없이 일반 반복 호출은 최대 10초
...
자동 ATR 포지션 위험 복기는 마지막 관측 참고값으로 열되 손절·실전 계산은
