...
| 오늘의 훈련 | 전략 v2, 10문제를 모두 서로 다른 유형으로 고정 |
| 자동 검증 | `npm run test:release` 성공 — 69/69 및 Chromium·오프라인 검증 통과 |
| PWA 캐시 | `v2.6.1-retire-t26.1` |
...
사용합니다. 전략 v2부터는 10문제 모두 서로 다른 유형으로 구성하며, 과거에
저장한 큐에 같은 유형이 여러 번 있으면 첫 항목은 유지하고 중복 위치만 교체합니다.
