...
| Volatility 포지션 경고 | 진입가·방향·현재/최대 계약·진입 고정 5분 ATR·보유시간으로 분석된 5개 손실 위험 조건을 개별 판정하고 위험 단계에서 축소·청산 검토를 표시 |
| 자동 검증 | `npm run test:release` 성공 — Node 208/208 및 Chromium·오프라인·320/390px 검증 통과 |
| 배포 공급망 | GitHub Actions 5개를 검증된 전체 커밋 SHA로 고정; Dependabot 주간 업데이트 제안; Linux CI Chrome 경로 고정 |
| PWA 캐시 | `v3.7.0-volatility-position-risk.1` — 포지션 5조건 위험 경고와 최근 완료 세션 읽기 전용 기준 포함 |
...
현재 포지션 입력에는 최대 보유 계약 수도 함께 기록합니다. 앱은 완결 거래
복기에서 정한 `1시간 손실`, `4시간·−2.25 ATR`, `12시간·−1.5 ATR·최대
5계약`, `24시간 보유`, `현재/최대 7계약`의 다섯 조건을 각 행에서 판정하고,
위험 단계에 따라 추가 진입 중단과 축소·청산 검토를 안내합니다. ATR은 진입
시 고정값이며 검증된 진행 MNQ 또는 직접 확인한 수동 시세에서만 판정합니다.
이는 과거 패턴 기반 자기 규칙 점검이지 미래 손실 확률이나 자동 주문이 아닙니다.
...
보여 줍니다. 이 값은 현재 시세가 아니므로 ATR·포지션·손절·실전 계산에는
사용하지 않습니다. 평균 가격은 범위 예산 참고선이고 조건부 선은 마감 후 복기
전용입니다. 수동 입력 패널은 기본적으로 닫혀 있습니다.
...
이전 주여도 최근 시가에 환산한 상승·하락 참고선을 기준시각과 함께 표시하되,
ATR·포지션·손절·실전 계산은 계속 잠급니다. 완료 5분봉이 정확히 1개 누락됐어도 H/L/current/시각의
...
읽기 전용 O/H/L/마지막 관측가와 현재 주간 상승·하락 환산선 참고에만 쓰고
ATR·포지션·손절 계산은 열지 않습니다.
