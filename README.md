...
| 자동 검증 | `npm run test:release` 성공 — Node 220/220 및 Chromium·오프라인·320/390px 검증 통과 |
| 배포 공급망 | GitHub Actions 5개를 검증된 전체 커밋 SHA로 고정; Dependabot 주간 업데이트 제안; Linux CI Chrome 경로 고정 |
| PWA 캐시 | `v3.11.0-volatility-tail-loss-avoidance.1` — 대손실 체크리스트 UI와 판정 엔진 배포 |
...
표시해 상단 주간 기준표만 환산합니다. 하단 실전 시가 계산은 계속 잠급니다. 시작 3개 이상이나
중간 둘 이상이 누락되거나
