...
문제은행은 기존 기초 120문항, 심화 232문항, 신규 오리지널 300문항을
합친 25개 유형·652문항입니다. 일반지식형 T19는 폐기하고 문자·기호 규칙
추론 T26을 추가했습니다. T23 스트룹만 보조 지표로 표시하며 나머지는 핵심
추론 정확도에 합산합니다. 모든 문항에는 규칙·적용·검산으로 나눈 해설,
2단계 힌트, 다차원 난이도 프로필과 선택지별 피드백이 연결되어 있습니다.
...
해시로 고정한 신규 300문항 소스를 병합하고 중복·폐기 정책을 적용한 뒤
실행용 JSON과 `answer-key.csv`를 재생성합니다.
...
`npm run enrich:content`를 실행합니다. `enrich:content`는 현재 652문제의
인지 영역, 오류 분류, 구조화 해설,
