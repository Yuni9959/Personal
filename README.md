...
| PWA 캐시 | `v3.11.1-volatility-reference-time.1` — 최근 완료 세션 선택과 기준 시가 원천시각 수정 |
...
Volatility 자동 배포가 2026-08-27 정상 시작 세션에서 멈춘 원인은 매번 바뀌는
생성 스냅샷을 음성 테스트 fixture로 사용하면서 `firstObservedBarAt`을 이미 같은
`session.start`로 바꾸던 데이터 의존 테스트였습니다. 위조 시각을 항상 실제로
변경하도록 고치고 정상 시작·시작 2봉 결손·중간 1봉 결손을 고정 fixture로
분리했습니다. 브라우저 검증은 주말 최초 진입의 로컬 참고와 사용자가 누른
`오늘 시세 새로고침`의 원격 우선 요청을 분리해, 버튼 요청이 최신 원천시각
후보를 반영하는 경로를 직접 확인합니다. 시세 신규도·429 대기·저장소 검증의
fail-closed 정책은 유지합니다.
...
`Volatility`는 실행 당일 한국 날짜로 이번 주 월요일을 구하고 직전 5년의
NQ 연속선물 프록시만 사용해 주간 계약을 자동 생성합니다. 현재 2026-08-24
기준 평균 H−L 범위는 양봉 1.758%·음봉 1.960%입니다. 장중 실전 기본선은
방향을 모르는 전체 거래일의 q25인 상승 +0.360%·하락 −0.295%이며, 최종
미사용 52주 도달률은 74.3%·73.1%입니다. 사후 양봉·음봉 조건부 복기선
+0.712%·−0.809%의 조건부 도달률은 80.74%·77.12%입니다.
조건부 선은 종가 방향을 사후에 알아야 하므로 장중 기본선으로 사용하지
않습니다. 화면의 모든 도달률과 포지션
손익은 가격 임계선 시나리오이지 거래 성공률·목표가·통계적 기대수익이
아닙니다.

현재 포지션에는 방향·평균 체결가격·체결시간을 기본으로 입력하고, 현재 계약 수·
거래 중 최대 계약 수·추가 진입 횟수를 더하면 대손실 판정이 완성됩니다. 현재가와
최신 연속 완료 5분봉의 Wilder ATR(14), 진입 뒤 MFE·MAE는 자동으로 반영됩니다.
797개 완결 거래 중 손실 60건(7.53%)이 승리 거래 총이익의 78.3%를 훼손한
분석을 바탕으로 `현재 불리한 변동`, `−1.84 ATR`, `2시간 손실`, `MAE −3.09
ATR`, `현재 3계약`, `최대 6계약`, `추가 진입 3회`, `시간·손실 복합 단계`의
8항목을 각각 검증합니다. `60분 경과 + 평가손익 −$500 이하`는 적색 실험
게이트로 표시합니다.

시간순 미래구간 검증에서 최고 위험등급은 손실률 40.0%, 전체 손실금액의
59.3%를 포함했지만, 모델 기반 전량청산은 순손익을 개선하지 못했습니다.
따라서 적색은 자동 주문 신호가 아니라 **추가 진입 금지 → 기존 손절·축소 계획
즉시 재평가** 순서의 경고입니다. 입력하지 않은 계약·경로 항목은 안전으로
간주하지 않습니다.

같은 최근 30일 완료 5분봉으로 EMA50·EMA200·RSI14·Wilder ATR과 최근 20세션
ATR% 백분위를 계산하고, 진입 뒤 MFE·MAE를 구합니다. 이 상태를 797개 완결
거래의 GMM 4군집 특성과 대조해 패턴 1~4 중 현재 경향과 군집별 주의점을
보여 줍니다. 현재 거래의 최종 결과를 모르고 규칙 기반으로 근사하므로 이는
정확한 GMM 재분류나 손실 확률이 아니라 설명 가능한 경향 판정입니다. 이 보조
판정은 별도 패널 대신 대손실 회피 화면 안에서 같은 포지션 입력을 공유합니다.

**OOS=가격선 도달률≠매매 성공률 · 조건부=마감 후 복기**

Volatility에 들어오면 첫 화면의 **이번 주 고정 기준** 상승·하락 2열 표에서
검증된 오늘 시가와 함께 평균 범위, 장중 기본선, 마감 조건부 복기선을 각각
`% / 시가 대비 pt / 시가 환산 가격`으로 바로 확인할 수 있습니다. 시작할 때는
이 기기의 검증 캐시를 먼저 다시 판정해 새 조회 전에도 직전 값을 보여 주되,
새 공급자 응답이 성공하기 전까지는 항상 읽기 전용으로 계산을 잠급니다.
승인된 완료 세션이 종료 전 마지막 5분까지 관측됐고 원천시각 96시간 이내면
**최근 완료 세션 참고값**으로 표시합니다. 새 주가 시작돼 원천일이 이전 주여도
현재 주간 기준을 최근 세션 시가에 환산한 상승·하락선을 원천 기준시각과 함께
보여 줍니다. 이 값은 현재 시세가 아니므로 가격선·손절·실전 계산에는
사용하지 않습니다. 다만 검증된 최신 연속 5분봉 ATR과 마지막 관측가는
현재 포지션 위험 복기에만 출처와 기준시각을 표시해 사용할 수 있습니다.
완료 세션의 시작 5분봉이 최대 2개 없으면 이전 세션으로 후퇴하지 않고 첫 유효
5분봉을 `최근 첫 관측 기준가 · 공식 시가 아님`으로 표시합니다. 기준가 옆 시각은
세션 마지막 관측시각이 아니라 그 가격이 실제로 나온 첫 관측시각입니다.
평균 가격은 범위 예산 참고선이고 조건부 선은 마감 후 복기 전용입니다. 수동
입력 패널은 기본적으로 닫혀 있습니다.
...
Volatility 페이지는 시작 시 이 기기의 검증 캐시를 먼저 다시 판정해 읽기
전용 참고값으로 표시하고, 최초
진입 또는 사용자가 **오늘 시세 새로고침** 버튼을 누를 때만
[Jina Reader](https://jina.ai/reader/)를 통해 `MNQ=F` 최근 30일치 5분봉 지연
프록시를 best-effort로 조회합니다. 주말 최초 진입은 동기화한 로컬 NQ를 먼저
읽지만 사용자가 버튼을 누르면 원격을 먼저 확인하고, 원격·로컬·현재 화면 중
원천시각이 가장 최근인 후보를 유지합니다. 시가 유무와 관계없이 일반 반복 호출은 최대 10초
...
pageshow에서도 네트워크를 다시 요청하지 않으며, 진행 세션 원천시각이 25분을
넘으면 거래 계산을 fail-closed합니다. 완료 세션은 마지막 5분 관측과 96시간
조건을 통과하면 `referenceOnly`로 표시합니다. 활성 주간 기준이 있으면 원천일이
이전 주여도 최근 시가에 환산한 상승·하락 참고선을 기준시각과 함께 표시합니다.
자동 지표·패턴 포지션 복기는 마지막 관측 참고값으로 열되 손절·실전 계산은
계속 잠급니다. 완료 5분봉이 정확히 1개 누락됐어도 H/L/current/시각의
공급자 메타 교차검증을 통과하고 결손 이후 연속 완료봉이 14개 이상이면 ATR을
그 구간에서 다시 계산합니다. 시가는
공급자 open 메타가 없으면 첫 세션봉 기준임을 표시합니다. 세션 시작의 완전-null
봉은 최대 2개까지 허용하고 첫 유효 5분봉을 `첫 관측 기준가 · 공식 시가 아님`으로
표시해 상단 주간 기준표만 환산합니다. 이 제한은 동기화된 로컬 완료 세션에도
같이 적용합니다. 하단 실전 시가 계산은 계속 잠급니다. 시작 3개 이상이나
중간 둘 이상이 누락되거나
...
실행하면 완료 세션 스냅샷과 주간 분석 계약이 함께 갱신됩니다. 주간 생성기는
`nasdaq_daily.csv`와 실행 당일의 한국 날짜에서 월요일~일요일을 구하고, 해당
월요일 직전 5년만 적합에 사용합니다. 생성 결과는
`apps/volatility/js/weekly-reference.generated.js`에 기록되므로 매주 소스 코드를
직접 수정하지 않습니다. NQ는 MNQ가 아니므로 로컬 완료 세션 값은 최대 96시간의
읽기 전용 O/H/L/마지막 관측가, 현재 주간 상승·하락 환산선과 자동 ATR
포지션 위험 복기에만 쓰고 손절·실전 계산은 열지 않습니다.
...
6. **Volatility 대손실 경고 모의운용 검증**: 신규 거래 최소 30~50건에서
   체크리스트 시각·위험점수·60분/−$500 게이트·가상 축소가·실제 종료 결과를
   함께 기록합니다. 최고 위험등급의 손실 포착과 회복 승자의 오경보 비용을
   다시 비교한 뒤에만 임계값이나 자동 손절 여부를 변경합니다. 자동 4패턴
   경향과 MFE·MAE 오분류도 같은 기록으로 점검합니다.
