...
| 항목 | 상태 |
| --- | --- |
| 확인일 | 2026-07-28 (Asia/Seoul) |
| 문제은행 | `2026.07.28-content.3`, 60개 유형·1,002문제·6,370개 보기 |
...
문제은행은 기초 120문항, 중복 제거 심화 232문항, 신규 오리지널 300문항,
Mensa Norway 공개 연습문항 35개 원형의 규칙을 바꿔 독자 생성한 350문항을
합친 60개 유형·1,002문항입니다. 새 문항은 `S01`~`S35`, 문항은 유형별
`Sxx-01`~`Sxx-10`을 사용합니다. 현재 652문항과 신규 350문항 사이의 ID,
동일 자극, 동일 보기 묶음과 완전 시각 중복은 모두 0건으로 검증했습니다.
일반지식형 T19는 폐기하고 문자·기호 규칙 추론 T26을 추가했습니다.
...
해시로 고정한 신규 300문항과 Mensa Norway 기반 신규 350문항 소스를
병합하고 ID·보기·유형 간 완전 시각 중복과 폐기 정책을 검사한 뒤 실행용
JSON과 `answer-key.csv`를 재생성합니다. ZIP에 포함된 구형 통합 650본은
현재 652문항의 부분집합을 포함하므로 사용하지 않고 신규 전용 350본만
원본으로 보존합니다.
...
`npm run enrich:content`를 실행합니다. `enrich:content`는 현재 1,002문제의
인지 영역, 오류 분류, 구조화 해설,
