...
| 자동 검증 | `npm run test:release` 성공 — Node 203/203 및 Chromium·오프라인·320/390px 검증 통과 |
| 배포 공급망 | GitHub Actions 5개를 검증된 전체 커밋 SHA로 고정; Dependabot 주간 업데이트 제안; Linux CI Chrome 경로 고정 |
| PWA 캐시 | `v3.6.1-volatility-recent-reference.1` — 최근 96시간 기준시각·상승/하락 읽기 전용 환산선과 자동 생성 주간 계약 포함 |
