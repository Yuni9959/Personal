...
| Volatility 시세·기준 | 주말에는 동기화한 최근 완료 `NQ=F` 세션을 우선 표시하고 평일 외부 MNQ 실패 때 참고 폴백; 주간 분석 기준은 `nasdaq_daily.csv`와 실행 당일 한국 날짜로 자동 생성 |
| 자동 검증 | `npm run test:release` 성공 — Node 202/202 및 Chromium·오프라인·320/390px 검증 통과 |
| 배포 공급망 | GitHub Actions 5개를 검증된 전체 커밋 SHA로 고정; Dependabot 주간 업데이트 제안; Linux CI Chrome 경로 고정 |
| PWA 캐시 | `v3.6.0-volatility-weekly-reference.1` — 자동 생성 주간 계약을 포함하고 로컬 NQ 스냅샷은 online-first 갱신·offline fallback |
...
승인된 MNQ 완료 세션이 종료 전 마지막 5분까지 관측됐고 원천시각 96시간 이내며
원천일과 현재가 같은 weekly 기준 기간일 때만 **최근 완료 세션 참고값**으로
표시합니다. 이 참고값은 현재 시세가 아니므로 ATR·포지션·손절·실전 계산에는
...
넘으면 거래 계산을 fail-closed합니다. 완료 세션은 마지막 5분 관측·96시간·
동일 weekly 기간을 모두 통과할 때만 `referenceOnly`로 표시하며 ATR·포지션·
손절·실전 계산은 계속 잠급니다. 완료 5분봉이 정확히 1개 누락됐어도 H/L/current/시각의
